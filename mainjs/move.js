$(document).ready(function() {
  ...
  $('.btn1').css({'background':'#fff','outline-color':'#fff'}); //첫번째 불켜
 // $('.btn1').css('width','30px'); // 버튼의 너비 증가
  
  $('.gallery .link1').fadeIn('fast'); //첫번째 이미지 보인다..
  $('.gallery .link1 span').delay(1500).animate({top:250, opacity:1},'slow');
  ...
  //   }
    ...
    $('.mbutton').css({'background':'none','outline-color':'rgba(255,255,255,0)'}); //버튼불다꺼!!
   // $('.mbutton').css('width','16px'); // 버튼 원래의 너비
    $('.btn'+cnt).css({'background':'#fff','outline-color':'#fff'});//자신만 불켜
   // $('.btn'+cnt).css('width','30px');    

    $('.gallery li span').css('top',150).css('opacity',0);
 ...
 $('.mbutton').click(function(event){  //각각의 버튼 클릭시
   ...
   // $('.btn'+cnt).css('width','30px');    
    ...
    $('.gallery li span').css('top',150).css('opacity',0);
    $('.gallery .link'+cnt).find('span').delay(1000).animate({top:250, opacity:1},'fast');
